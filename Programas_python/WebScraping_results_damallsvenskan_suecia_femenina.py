...
url='https://www.livefutbol.com/calendario/swe-frauen-damallsvenskan-2022-spieltag/26/'
...
url_principal='https://www.livefutbol.com/calendario/swe-frauen-damallsvenskan-'
for años in (eq[1].find_all('form')[0].find_all('option')):
    añosarreglados = años.text
    añosarreglados = añosarreglados.replace('/', '-')
    if (añosarreglados == '2008-20asd09'):
        url_iterada ='https://www.livefutbol.com/calendario/_110/22/'
    else:
    ...
    for jornada in list(range(len(eq[1].find_all('form')[1].find_all('option'))-1,-1,-1)):
        ...
        añosarreglados=años.text
        añosarreglados=añosarreglados.replace('/','-')
        if (añosarreglados == '2008-h2009'):
            url_iterada = 'https://www.livefutbol.com/calendario/_110/'+jornada_numero[0]+'/'
        else:
...
df.to_csv('D:/MEGAsync/andrey u/Maestría/Tesis/datasets_results_f/damallsvenskan_secia_femenina_results',index=False)
print(df)
